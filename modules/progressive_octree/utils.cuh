...
/**
 * Calls function @ref `f` @ref `size` times, supplying @ref `f` with the unique index of the function call.
 * Calls are distributed evenly over all threads available in the current thread block.
 * @tparam Function A lambda of type `void(size_t)`
 * @param size the number of times to call the function
 * @param f the function to call
 */
template <typename Function>
void processRangeBlock(unsigned size, Function&& f) {
  auto block = cg::this_thread_block();
  for (size_t i = block.thread_rank(); i < size; i += block.num_threads()) {
    f(i);
  }
}
