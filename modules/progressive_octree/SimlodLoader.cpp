...
VOID CALLBACK
FileIOCompletionRoutine_simlod(__in DWORD errorCode, __in DWORD numBytesTransfered, __in LPOVERLAPPED lpOverlapped) {
  ...
  if (numBytesTransfered == 0) {
    spdlog::info("0 bytes read. offset: {:14L}, numBytes: {:9} \n", lpOverlapped->Offset, numBytesTransfered);
...
void loadFileNative(
  ...
  do {
    ...
    auto returnValue = ReadFileEx(hFile, (char*)target, numBytes_aligned, &ol, FileIOCompletionRoutine_simlod);
