	...
	__in LPOVERLAPPED lpOverlapped)
{
	...
	if(errorCode != 0){
		spdlog::error("read error: {}", errorCode);
	}

	if(numBytesTransfered == 0){
		spdlog::info("0 bytes read. offset: {:14L}, numBytes: {:9}", lpOverlapped->Offset, numBytesTransfered);
...
uint64_t loadLasNative(string file, LasHeader header, uint64_t firstPoint, uint64_t numPoints){
	...
	do{
		...
		if (returnValue == 0) {
			spdlog::error("ReadFileEx failed");
...
uint64_t loadLasNative(const std::string& file, LasHeader header, uint64_t firstPoint, uint64_t numPoints){
...
void loadLasNative(const std::string& file, LasHeader header, uint64_t firstPoint, uint64_t numPoints, void* target, const double translation[3], const double boundsMax[3]){
	...
	for(uint64_t i = 0; i < numPoints; i++){
        ...
        point.x = static_cast<float>(std::clamp(static_cast<double>(XYZ[0]) * scale_x + offset_x, 0.0, boundsMax[0]));
        point.y = static_cast<float>(std::clamp(static_cast<double>(XYZ[1]) * scale_y + offset_y, 0.0, boundsMax[1]));
        point.z = static_cast<float>(std::clamp(static_cast<double>(XYZ[2]) * scale_z + offset_z, 0.0, boundsMax[2]));

		if(offset_rgb > 0){
			auto* rgb = (uint16_t*)(source + header.bytesPerPoint * i + offset_rgb);
