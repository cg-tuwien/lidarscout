...
#ifndef IPES_CUDAVIRTUALMEMORY_H
#define IPES_CUDAVIRTUALMEMORY_H
...
struct CudaVirtualMemory {
	...
	static std::shared_ptr<CudaVirtualMemory> create(uint64_t virtualSize = DEFAULT_VIRTUAL_SIZE) {
		...
		if (result != CUDA_SUCCESS) {
			spdlog::error("error {} while trying to reserve virtual memory.", int(result));
			exit(52457);
		}
	...
	void commit(uint64_t requested_size) {
		...
		if (result != CUDA_SUCCESS) {
			spdlog::error("error {} while trying to allocate physical memory.", int(result));
			exit(52458);
		}
...
#endif	//IPES_CUDAVIRTUALMEMORY_H
