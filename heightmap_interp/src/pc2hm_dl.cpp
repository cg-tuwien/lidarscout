    ...
    c10::Dict<std::string, torch::Tensor>& dict)
{
	...
	{
            tensor_cpu = tensor_cpu.permute(permutation).contiguous();
        }
    ...
    int verbose_level)
{
    ...
    if (img_rgb_lin.size() > 0)
    {
        ...
        inputs.insert("patch_rgb_mask", inputs.at("patch_hm_mask").expand({-1, 3, -1, -1}).contiguous().clone());
    }

    // print names and sizes
    //for (const auto& item : inputs)
    //{
    //    std::cout << item.key() << ": " << item.value().sizes() << ": NaN=" << item.value().isnan().any() << std::endl;
    //}
