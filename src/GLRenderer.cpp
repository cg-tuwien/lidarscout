...
static void cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
	...
	global_controls->onMouseMove(xpos, ypos);
}
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
	...
	global_controls->onMouseScroll(xoffset, yoffset);
}
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
	...
	global_controls->onMouseButton(button, action, mods);
}

GLRenderer::GLRenderer() {
	this->controls = global_controls;
...
void GLRenderer::init() {
	...
	if (numMonitors > 1) {
		...
		window = glfwCreateWindow(1920, 1080, "Lidar Scout", nullptr, nullptr);
	...
	glfwSetDropCallback(window, [](GLFWwindow*, int count, const char** paths) {
		...
		println("glfwSetDropCallback", t_start);
	});
...
void GLRenderer::loop(const std::function<void(void)>& update, const std::function<void(void)>& render) {
	int fpsCounter = 0;
	double start = now();
	...
	while (!glfwWindowShouldClose(window)) {
		...
		{	// RENDER IMGUI PERFORMANCE WINDOW
			...
			ImGui::SetNextWindowSize(windowSize_perf, ImGuiCond_Once);

			ImGui::Begin("Performance");
			std::string fpsStr = rightPad("FPS:", 30) + strFps;
...
void Texture::setSize(int newWidth, int newHeight) {
	...
	if (needsResize) {
		...
		glTextureStorage2D(this->handle, 1, this->colorType, newWidth, newHeight);
